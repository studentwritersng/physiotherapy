...
- **Design tokens only.** After Task 1, use the `doc/clinic-dashboard.html` tokens — `bg-surface`, `text-ivory`, `text-ivory-dim`, `border-line`, `text-jade`, `bg-jade-dim`, `text-orchid`, `bg-orchid-dim`, `text-gold`, `text-sky`. `font-display` for headings, `tabular` for times, prices and percentages. No raw `gray-*`, `blue-*` or `cyan-*` utilities in new code.
...
## Task 1: Design tokens, themes and typography
...
**Interfaces:**
- Consumes: nothing
- Produces: Tailwind utilities from the `@theme` tokens — `bg-ink`, `bg-surface`, `bg-surface-2`, `text-ivory`, `text-ivory-dim`, `text-ivory-faint`, `border-line`, `text-jade`, `bg-jade-dim`, `text-gold`, `bg-gold-dim`, `text-orchid`, `bg-orchid-dim`, `text-sky`, `bg-sky-dim`, `rounded-lg`/`rounded-md`/`rounded-sm` at the mockup's radii. Fonts: `font-display` (Fraunces) and `font-sans` (Space Grotesk). Also `src/components/ThemeToggle.tsx` exporting `ThemeToggle()`.

**The design reference is `doc/clinic-dashboard.html`.** Read it before starting — it is a complete, dependency-free mockup and every value below is lifted from it. `AGENTS.md` summarises it.
...
/*
 * Design tokens from doc/clinic-dashboard.html. Deep jade-green and ivory.
 *
 * Light is the default, so its values sit on :root and dark is opt-in via
 * html[data-theme="dark"]. The mockup is arranged the other way round; this
 * inversion avoids a flash of dark before hydration.
 *
 * Tailwind v4 is CSS-first: a name under @theme becomes a utility, so
 * --color-jade yields text-jade, bg-jade and border-jade.
 */
@theme {
  --color-ink: #fbf7ee;
  --color-ink-2: #f3eddf;
  --color-surface: #ffffff;
  --color-surface-2: #f6f1e4;
  --color-surface-3: #efe7d2;
  --color-line: rgba(20, 40, 32, 0.1);
  --color-line-strong: rgba(20, 40, 32, 0.17);
  --color-ivory: #16302a;
  --color-ivory-dim: rgba(22, 48, 42, 0.64);
  --color-ivory-faint: rgba(22, 48, 42, 0.42);
  --color-jade: #1e9c73;
  --color-jade-dim: rgba(30, 156, 115, 0.12);
  --color-gold: #b87517;
  --color-gold-dim: rgba(184, 117, 23, 0.12);
  --color-orchid: #ad3d6d;
  --color-orchid-dim: rgba(173, 61, 109, 0.12);
  --color-sky: #25839f;
  --color-sky-dim: rgba(37, 131, 159, 0.12);
  --color-track: rgba(22, 48, 42, 0.08);
...
/*
 * Dark theme. Tailwind's @theme cannot vary by selector, so the raw custom
 * properties are overridden here and the utilities follow automatically.
 */
html[data-theme="dark"] {
  --color-ink: #0a1d18;
  --color-ink-2: #0e2620;
  --color-surface: #122c25;
  --color-surface-2: #183931;
  --color-surface-3: #1e453a;
  --color-line: rgba(245, 239, 227, 0.08);
  --color-line-strong: rgba(245, 239, 227, 0.14);
  --color-ivory: #f5efe3;
  --color-ivory-dim: rgba(245, 239, 227, 0.62);
  --color-ivory-faint: rgba(245, 239, 227, 0.38);
  --color-jade: #33c793;
  --color-jade-dim: rgba(51, 199, 147, 0.16);
  --color-gold: #f0a83b;
  --color-gold-dim: rgba(240, 168, 59, 0.16);
  --color-orchid: #d9598e;
  --color-orchid-dim: rgba(217, 89, 142, 0.16);
  --color-sky: #5fb8d6;
  --color-sky-dim: rgba(95, 184, 214, 0.16);
  --color-track: rgba(245, 239, 227, 0.08);
}

/* The mockup's two soft radial washes over the page background. */
body {
  background:
    radial-gradient(1100px 620px at 84% -8%, var(--color-jade-dim), transparent 60%),
    radial-gradient(900px 500px at -6% 8%, var(--color-orchid-dim), transparent 55%),
    var(--color-ink);
  min-height: 100vh;
}
...
/* Amounts, times and percentages line up in a column. */
.tabular {
...
/* The mockup's staggered card entry. */
...
/**
 * Sets data-theme on <html>, which is what the CSS custom properties key off.
 * The choice persists in localStorage; light stays the default when nothing is
 * stored, matching the server-rendered markup so there is no flash.
 */
...
- card wrapper: `rounded-lg border border-line bg-surface p-7 shadow-[0_18px_34px_-22px_rgba(20,40,32,0.22)]`
- heading: add `font-display` and `text-ivory`, size `text-2xl`
- subtitle: `text-ivory-dim`
- error banner: `rounded-md bg-orchid-dim px-3.5 py-2.5 text-sm font-medium text-orchid`
- submit button: `min-h-11 cursor-pointer rounded-md bg-jade px-5 py-2.5 text-base font-semibold text-white transition-opacity duration-200 hover:opacity-90 disabled:opacity-60`
- footer links: `font-medium text-jade hover:opacity-80`

In `src/components/NavShell.tsx`:

- aside: `border-line bg-gradient-to-b from-ink-2 to-ink md:w-64`
- brand name: `font-display text-lg font-semibold text-ivory`
- role line: `text-xs uppercase tracking-[0.14em] text-ivory-faint`
- active link: `rounded-md bg-gradient-to-r from-jade-dim to-transparent px-3 py-2.5 text-sm font-medium text-ivory`
- inactive link: `rounded-md px-3 py-2.5 text-sm font-medium text-ivory-dim transition-colors duration-150 hover:bg-surface hover:text-ivory`, plus `cursor-pointer`
- disabled span: `text-ivory-faint`, badge `rounded bg-track px-1.5 py-0.5 text-[10px] uppercase tracking-wider`
- add `<ThemeToggle />` beside the logout button, importing it from `@/components/ThemeToggle`

In `src/components/LogoutButton.tsx` and `src/app/page.tsx`: `text-jade hover:opacity-80` plus `cursor-pointer`.
...
Expected: 200. Then open `http://localhost:3200/login` in a browser, confirm the ivory background with jade accents renders, click the theme toggle and confirm dark applies and survives a reload.

**Measure, do not assume:** with devtools, check the computed contrast of `--color-ivory` on `--color-surface` (light) and again in dark. Both must be at least 4.5:1. Also check `--color-ivory-dim` on `--color-surface`. Record the actual ratios in your report. If any falls below 4.5:1, report it as a concern with the measured number rather than adjusting the palette yourself — the tokens come from the client's mockup.
...
git commit -m "feat: land the clinic-dashboard design system

Tokens, both themes and typography from doc/clinic-dashboard.html: deep
jade-green and ivory, Fraunces for display and Space Grotesk for UI, the
mockup's radii, its two radial background washes and its staggered card entry.

Light is the default, so light values sit on :root and dark is opt-in via
html[data-theme=\"dark\"]. The mockup is arranged the other way round; this
inversion avoids a flash of dark before hydration.

Fonts load through next/font so they are self-hosted — no third-party request
on the critical path and no layout shift.

Retro-fits the Foundation auth screens off raw gray and blue utilities, adds a
theme toggle, 44px minimum touch targets and a single global :focus-visible
ring. The Playwright login journeys assert on roles and accessible names, so
they cover this restyle unchanged."
```
...
export default async function SettingsLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <header>
        <h1 className="text-2xl font-semibold text-ivory">Clinic settings</h1>
        ...
        <ul className="flex flex-wrap gap-1">
          {TABS.map((tab) => (
            <li key={tab.href}>
              <Link
                href={tab.href}
                className="inline-flex min-h-11 cursor-pointer items-center rounded-t-md px-4 py-2 text-sm font-medium text-ivory transition-colors duration-150 hover:bg-surface-2"
              >
                {tab.label}
              </Link>
            </li>
            ...
            <li key={day} className="flex flex-wrap items-end gap-3">
              ...
              <div className="flex flex-col gap-1">
                ...
                <input
                  ...
                  className="min-h-11 rounded-md border border-line bg-white px-3 py-2 tabular text-base disabled:text-ivory-faint"
                />
              </div>
              ...
              {!isOpen && <span className="pb-3 text-sm text-ivory-faint">Closed</span>}
              {dayError && (
          ...
          {services.map((service) => (
            <tr key={service.id} className="border-b border-line">
              ...
              <td className="py-3 pr-4">
                {service.active ? (
                  <span className="rounded bg-jade-dim px-2 py-1 text-xs font-medium text-jade">
                    Active
                  </span>
                ) : (
                  <span className="rounded bg-surface-2 px-2 py-1 text-xs font-medium text-ivory-faint">
                    Inactive
                  </span>
                )}
              </td>
...
in tabular figures, which is what the `tabular` class is for."
```
...
export function AvailabilityForm({
  ...
  const inputClass =
"min-h-11 rounded-md border border-line bg-white px-3 py-2 tabular text-base";
    ...
    <form action={formAction} className="flex flex-col gap-4">
      ...
      <label className="flex cursor-pointer items-center gap-2 text-sm">
        <input
          ...
          name="isBlocked"
          value="true"
          className="size-5 cursor-pointer accent-orchid"
        />
      ...
      <div className="flex flex-col gap-1">
        ...
        <input
          ...
          className="min-h-11 rounded-md border border-line bg-white px-3 py-2 text-base"
        />
      </div>
  ...
  return (
    <div className="flex flex-col gap-6">
      <section>
        ...
        {recurring.length === 0 ? (
          <p className="mt-2 text-sm text-ivory-faint">No weekly hours.</p>
        ) : (
          <ul className="mt-2 flex flex-col gap-2">
            {recurring.map((row) => (
              ...
              >
                ...
                {row.reason && <span className="text-sm text-ivory-faint">{row.reason}</span>}
      ...
      <section>
        ...
        {dated.length === 0 ? (
          <p className="mt-2 text-sm text-ivory-faint">No dated entries.</p>
        ) : (
          <ul className="mt-2 flex flex-col gap-2">
            {dated.map((row) => (
              <li
            ...
            return (
              <li key={therapist.id}>
                <Link
                  ...
                  className={`inline-flex min-h-11 cursor-pointer items-center rounded-md px-4 py-2 text-sm font-medium transition-colors duration-150  ${
                    isSelected
                      ?"bg-jade text-white"
